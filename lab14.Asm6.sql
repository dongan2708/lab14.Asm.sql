...
SELECT BookName
FROM Books
WHERE BooklistID IN (SELECT BooklistID FROM Booklist WHERE BooklistName = N'Văn Học Dân Gian')
--6. Liệt kê các cuốn sách có tên bắt đầu với chữ “T” theo thứ tự giá giảm dần
SELECT BookName,Price
FROM Books
WHERE BookName like'T%'ORDER BY Price DESC
...
WHERE PublisherName = N'Tri Thức'GROUP BY BookName 
--15. Thống kê số đầu sách của mỗi loại sách
SELECT BookName,count(*) as Sum_books
FROM Books as B
JOIN Booklist as Bl
ON Bl.BooklistID= B.BooklistID
WHERE BooklistName =  N'Khoa Học Xã Hội'GROUP BY BookName
...
--18. Viết Store Procedure:
 ...
 CREATE PROCEDURE SP_Them_Sach AS
 INSERT INTO Books VALUES ('B002', N'Trí Tuệ Nhân Tạo', '2019', 11,1,111, 200000,1000,5) 
 --SP_Tim_Sach: Tìm các cuốn sách theo từ khóa
 CREATE PROCEDURE SP_Tim_Sach AS
 SELECT * FROM VIEW_Books  
 --SP_Sach_ChuyenMuc: Liệt kê các cuốn sách theo mã chuyên mục
 CREATE PROCEDURE SP_Sach_ChuyenMuc AS
 SELECT BooklistID, BookName  FROM Books
 ...
 AS  
	BEGIN 
		 IF EXISTS (SELECT * FROM deleted WHERE BookListID IN(
		 SELECT BookListID FROM Books WHERE Timesofpublication >0))	  
		 BEGIN       
